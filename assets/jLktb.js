...
for(var i=0;i < ca.length;i++) {
	...
	if (c.indexOf(nameEQ) == 0){			
		var tempCook = decodeURIComponent(c.substring(nameEQ.length,c.length));
		var tmp = eval('(' + tempCook + ')');
			if (tmp) {			
			  if(Number(tmp.frequency) > maxTmp){
			    maxTmp = Number(tmp.frequency);
			    maxCookie = tempCook;
			  }
			} 
	}
...
//APE.Config.scripts = [APE.Config.baseUrl+'/Build/closureCompiler/apeCoreSession13.js'];

(function(){
	for (var i = 0; i < arguments.length; i++)
		APE.Config.scripts.push(APE.Config.baseUrl + '/Source/' + arguments[i] + '.js');
})('mootools-core-141', 'Core/APE', 'Core/Events', 'Core/Core', 'Pipe/Pipe', 'Pipe/PipeProxy', 'Pipe/PipeMulti', 'Pipe/PipeSingle', 'Request/Request','Request/Request.Stack', 'Request/Request.CycledStack', 'Transport/Transport.longPolling','Transport/Transport.SSE', 'Transport/Transport.XHRStreaming', 'Transport/Transport.JSONP', 'Transport/Transport.WebSocket', 'Core/Utility', 'Core/JSON');
